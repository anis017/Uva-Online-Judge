...
void Run_Case() 
{
   ...
   while (cin >> n and n != 0) {
      ...
      for (lls i = 3; i * i <= n; i += 2) {
         while (n % i == 0) {
            if (neg) ans += " x ", ans += to_string(i);
            else ans += " ", ans += to_string(i), ans += " x";
            n /= i;
         }
      }

      if (neg) {
         if (n > 1) ans += " x ", ans += to_string(n);
      }
      else if (n > 1) ans += " ", ans += to_string(n);
      while (ans.back() == 'x' or ans.back() == ' ') ans.pop_back();
      cout << ans << N;
   }
}
