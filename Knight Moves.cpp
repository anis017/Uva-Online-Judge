...
void bfs(int a, int b, int c, int d) {
    ...
    while (!q.empty()) {
        int cur_x = q.front().ff, cur_y = q.front().ss;
        q.pop();

        if (cur_x == c and cur_y == d) {
            cout << "To get from " << s << " to " << t << " takes " << cost[cur_x][cur_y] << 
            " knight moves." << N; 
            return;
        }
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if ((xx >= 1 and xx <= 8) and (yy >= 1 and yy <= 8) and !vis[xx][yy]) {
                vis[xx][yy] = 1;
                cost[xx][yy] = cost[cur_x][cur_y] + 1;
                q.push({xx, yy});
            }
        }
    }
